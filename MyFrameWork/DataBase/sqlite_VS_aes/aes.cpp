...
void aes_gen_tables()
{
    ...
    for( i = 0, x = 1; i < 10; i++, x = (unsigned char) XTIME(x ))
    {
        RCON[i] = (uint32) x << 24;
    }
    ...
    for( i = 0; i < 256; i++ )
    {
        x = (unsigned char) FSb[i]; y = (unsigned char) XTIME(x );
...
unsigned char aes::default_key[32] =
        {
...
int aes:: aes_set_key(uint8 *key, int nbits )
{
    int i;
    uint32 *RK, *SK;
...
void aes:: aes_encrypt(uint8 input[16], uint8 output[16] )
{
...
void aes:: aes_decrypt(uint8 input[16], uint8 output[16] )
{
    uint32 *RK, X0, X1, X2, X3, Y0, Y1, Y2, Y3;
